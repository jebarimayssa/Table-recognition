...
def imageToTable(image_path):
                ...
                for k in range(len(finallist[i][j])):
                    ...
                    if(len(out) == 0):
                        ...
                        out = pytesseract.image_to_string(
                            img, config=custom_oem_psm_config)
    ...
    #Transformation pour les valeurs négatives
    dataJson = dataJson.replace("(", "-")
    dataJson = dataJson.replace(")", " ")



    download_path = os.path.join(MEDIA_ROOT, 'downloadData')
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    data = dataframe.style.set_properties(**{'text-align': 'left'})
    excel_file_path = os.path.join(MEDIA_ROOT, 'downloadData', 'outputX.xlsx')
    csv_file_path = os.path.join(MEDIA_ROOT, 'downloadData', 'outputC.csv')
    json_file_path = os.path.join(MEDIA_ROOT, 'downloadData', 'outputJ.json')
